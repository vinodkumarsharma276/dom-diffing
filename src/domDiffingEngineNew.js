const compareDomsNew = (baseLineDom, candidateDom) => {
    ...
    for(let i=0; i<candidateTags.length; i++){
        if(candidateTags[i] == baseLineTags[i]){
            ...
            cssComparisonResult = compareNodeCSSNew(baseLineDom[baseLineTags[i]]["cssProps"], candidateDom[candidateTags[i]]["cssProps"]);
            ...
            for(let j=0; j<candidateDomChildNodes.length; j++){
                if(baseLineChildNodes[j]){
                    compareDomsNew(baseLineChildNodes[j], candidateDomChildNodes[j]);
                }
            }
        }
    }
...
const compareNodeCSSNew = (baseLineCSS, candidateCSS) => {
